...
見た目は **「船長のコンソール」** をテーマにしたスキューモーフィズム（黒漆の木目・真鍮の金具・ロープの仕切り・港の水面）。テクスチャはすべてインラインSVGで内蔵し、外部通信はしません。
...
- 質感・配色: `sidepanel.css` の `:root` 変数（`--brass-*` = 真鍮, `--anchor` = 琥珀/恒久, `--live`/`--water` = 寒色/流動, `--rope` = ロープ）
